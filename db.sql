...
GRANT SELECT, UPDATE ON TABLE test.user TO user_login;
GRANT ALL ON TABLE test.user_meme TO user_login;
...
CREATE POLICY insert_user_meme on test.user_meme for INSERT TO user_login WITH CHECK (id = test_public.get_user_id()); -- Give access to every user to insert memes for themselves
CREATE POLICY update_user_meme ON test.user_meme FOR UPDATE TO user_login USING (id = test_public.get_user_id());
CREATE POLICY delete_user_meme ON test.user_meme FOR DELETE TO user_login USING (id = test_public.get_user_id());
